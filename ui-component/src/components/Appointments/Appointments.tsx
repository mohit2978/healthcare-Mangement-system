    ...
    const fetchData = () => {
        ...
        if (role === "PATIENT") {
            ...
            if (user && user.id) {
                ...
                console.warn("User ID not found for PATIENT.");
                setShowSpinner(false);
            }
        } else if (role === "DOCTOR") {
            if (user && user.id) {
                ...
                console.warn("User ID not found for DOCTOR.");
                setShowSpinner(false);
            }
        } else {
                    ...
                    <div className="card-body">
                        <div className="d-flex justify-content-between">
                            ...
                            <div className="d-flex flex-column justify-content-between align-items-center">
								...
								{currentRole === "PATIENT" && !isPast(appointment.appointmentTime) && appointment.status !== "REJECTED" && (
									<button className="bg-transparent border-0 p-1">
								...
								{currentRole === "DOCTOR" && !isPast(appointment.appointmentTime) && appointment.status === "CONFIRMED" && appointment?.doctorComments?.length === 0 && (
									<button className="bg-transparent border-0 p-1">
										<FontAwesomeIcon icon={faPencil} className="mx-1" onClick={() => updateModalShow()} />
									</button>
								)}
								{/* Receiving new appointment (Confirm or reject) */}
								{currentRole === "DOCTOR" && !isPast(appointment.appointmentTime) && appointment.status === "PENDING" && (
									<div>
										<button className="bg-transparent border-0 p-1">
