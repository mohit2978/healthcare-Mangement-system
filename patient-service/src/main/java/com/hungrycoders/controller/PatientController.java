...
public class PatientController {
    ...
    public ResponseEntity<?> getPatientById(@PathVariable String id) throws ResourceNotFoundException {
        try {
            ...
            return ResponseEntity.status(200)
                    .body(new GenericResponse<>("Patient fetched successfully", patientService.getPatientById(id)));
        } catch (Exception e) {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<?> updatePatient(@Valid @RequestBody Patient patient, @PathVariable String id)
            throws Exception {
        try {
            com.hungrycoders.model.Patient updatedPatient = patientService.updatePatientById(id, patient);
            return ResponseEntity.status(200)
                    .body(new GenericResponse<>("Patient updated successfully", updatedPatient));
        } catch (Exception e) {
